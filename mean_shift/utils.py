...
def generate_responses_2():
    ...
    return gausssmooth(responses, 7)
...
def custom_get_patch(x, y, kernel_h, kernel_w, img):
    x = max(int(x), 0)
    y = max(int(y), 0)
    x = min(x, img.shape[1] - kernel_w)
    y = min(y, img.shape[0] - kernel_h)
    return x, y, img[y : y + kernel_h, x : x + kernel_w]
