...
from particle_filter import ParticleFilter, PFParams
from sequence_utils import VOTSequence

# set the path to directory where you have the sequences
dataset_path = "vot2014/"  # set to the dataet path on your disk
sequences = os.listdir(dataset_path)  # Choose all sequences

# sequences = ["david"]  # choose the sequence you want to test
...
for sequence in sequences:
    if (
        sequence == "list.txt" or sequence == "ball" or sequence == "car"
    ):  # ball seq has error in it
        continue
    ...
    parameters = PFParams(NCVKalmanFilter, 100, 0.5, 1, 32, 0.025, use_ncc=True)
    tracker = ParticleFilter(parameters)
    ...
    ao = sum(avg_oclussion) / len(avg_oclussion) * 100
    fps = sequence.length() / time_all
    print("Average overlap: %.2f" % (sum(avg_oclussion) / len(avg_oclussion) * 100))
    print("Tracking speed: %.1f FPS" % (sequence.length() / time_all))
    print("Tracker failed %d times" % n_failures)
    overall_fail += n_failures
    overall_ao += ao
    overall_fps += fps

print("Overall average overlap: %.2f" % (overall_ao / len(sequences)))
print("Overall average FPS: %.1f" % (overall_fps / len(sequences)))
print("Overall fails: %d" % overall_fail)
