...
def sample_random(im_sz, n):
    # sample n samples from a uniform distribution
    return np.random.uniform(low=[0, 0], high=[im_sz], size=(n, 2))
...
class TrackerSiamFC(Tracker):
    ...
    def __init__(self, net_path=None, **kwargs):
        ...
        self.n_samples = 1
        self.re_detection = False
        self.n_redetection = 0
        self.sigma_factor = 1
    ...
    def init(self, img, box):
        ...
        box = np.array(
            [box[1] - 1 + (box[3] - 1) / 2, box[0] - 1 + (box[2] - 1) / 2, box[3], box[2]],
            dtype=np.float32,
        )
    ...
    def get_max_response(self, x):
        ...
        responses = np.stack(
            [
                cv2.resize(u, (self.upscale_sz, self.upscale_sz), interpolation=cv2.INTER_CUBIC)
                for u in responses
            ]
        )
        ...
        best_id = np.argmax(np.amax(responses, axis=(1, 2)))

        print("Best ID: ", best_id)

        # peak location
        response = responses[best_id]

        return max(0, response.max()), best_id, response
    ...
    def update(self, img):
        ...
        if not self.re_detection:
            ...
            print("Target lost...")
            sigma = max(self.target_sz) * 2

            if self.sampling_method == "gaussian_grow":
                sigma *= self.sigma_factor

            # proposals = sample_gauss(self.center, np.diag([sigma, sigma]), n=10)
            proposals = sample_random(img.shape[:2], n=10)
            ...
            max_resp, best_id, response = self.get_max_response(x)
            scale_id = 1
        ...
        if max_resp < 5:
            max_resp = 0
            self.re_detection = True
            self.n_redetection += 1
        else:
            self.re_detection = False
        ...
        if not self.re_detection:
            ...
            disp_in_image = (
                disp_in_instance * self.x_sz * self.scale_factors[scale_id] / self.cfg.instance_sz
            )
        ...
        if self.sampling_method == "gaussian_grow":
            if self.re_detection:
                if self.n_redetection in [5, 10, 20, 30, 50, 100, 250]:
                    self.sigma_factor *= 1.1
        ...
        if proposals is not None:
            return box, max_resp, proposals, self.target_sz
        else:
            return box, max_resp, None, None
